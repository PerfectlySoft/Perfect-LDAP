...
public class LDAP {
  ...
  String:Any]] {
    ...
    /// asynchronized search with sync result
    /// - parameters:
    /// - base: String, search base domain (dn), default = ""
    /// - filter: String, the filter of query, default = "(objectclass=*)", means all possible results
    /// - scope: See Scope, BASE, SINGLE_LEVEL, SUBTREE or CHILDREN
    /// - attributes: filter for output attributes
    /// - sortedBy: a sorting string, may be generated by LDAP.sortingString()
    /// - sizeLimit: max count of result search. Max limit is 1000.
    /// - returns:
    /// ResultSet. See ResultSet
    /// - throws:
    /// Exception.message
    public func asyncSearch(base:String = "", filter:String = "(objectclass=*)", scope:Scope = .BASE, attributes: [String] = [], sortedBy: String = "", sizeLimit: Int = 1000) throws -> [String:[
        String:Any]]
    {
        
        var realSizeLimit : Int32 = Int32(sizeLimit)
        
        // This is limit of space
        if realSizeLimit > 1000 {
            realSizeLimit = 1000
        }
        ...
        let r = withCArrayOfString(array: attributes) { pAttribute -> Int32 in
            ...
            // perform the search
            let result = ldap_search_ext(self.ldap, base, scope.rawValue, filter, pAttribute, 0, &serverControl, nil, &t, realSizeLimit, &msgid)
        ...
        var msgtype = LDAP_RES_SEARCH_ENTRY
        var msgcount = 0;
        while(msgtype != LDAP_RES_SEARCH_RESULT && realSizeLimit > msgcount)
        {
            var res = OpaquePointer(bitPattern: 0)
            msgcount += 1
            // retrieves result
            var err = ldap_result(ldap, msgid, 0, &t, &res);
            switch(err)
            {
            case -1:
                ldap_get_option(ldap, LDAP_OPT_RESULT_CODE, &err)
                print(stderr, "ldap_result(): %s\n", ldap_err2string(err))
                throw Exception.message(LDAP.error(err))
            case 0:
                print(stderr, "ldap_result(): timeout expired\n")
