...
public class LDAP {
  ...
  String:Any]] {
    ...
    /// - throws:
    ///   Exception.message
    public func asyncSearch(base:String = "", filter:String = "(objectclass=*)", scope:Scope = .BASE, attributes: [String] = [], sortedBy: String = "") throws -> [String:[
        String:Any]]
    {
        ...
        var msgcount = 0;
        while(msgtype != LDAP_RES_SEARCH_RESULT)
        {
            var res = OpaquePointer(bitPattern: 0)
            msgcount += 1
            // retrieves result
            var err = ldap_result(ldap, msgid, 0, nil, &res);
            switch(err)
            {
            case -1:
